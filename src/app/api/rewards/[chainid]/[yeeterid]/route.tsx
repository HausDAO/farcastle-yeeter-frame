...
import { formatRaiseStatsDate, toWholeUnits } from "@/lib/helpers";
  ...
  try {
    ...
    try {
      ...
      if (profileMatch?.content) {
        ...
        if (profile.rewards) {
          const rewardsProfile = addParsedRewards(profile);
          rewards = rewardsProfile?.parsedRewards;
      ...
      const uniqueAddresses = [...new Set([...yeeterAddresses])];
      const options = {
        method: "GET",
      ...
      (
        ...
        >
          ...
          >
            ...
            >
              {rewards?.map((reward) => {
                return <div>{`${reward.title} - ${reward.rewardLevel}`}</div>;
              })}
            </div>
